...
import {
  ...
  VoidrBrand,
  VoidrMark,
} from "@voidr/capture-design-system";
...
function launchErrorMessage(error: unknown): string {
  ...
  if (
    /must belong to an organization|pertencer a uma organização/i.test(message)
  ) {
...
function voiceSendErrorMessage(rawMessage: string): string {
  if (/área|contexto visual|selecione-a novamente/i.test(rawMessage))
    return rawMessage;
  ...
  if (
    /fetch|network|temporarily unavailable|indisponível|conexão/i.test(
      rawMessage,
    )
  ) {
...
function App() {
  ...
  const [voiceVisualFlow, setVoiceVisualFlow] = useState(
    initialVoiceVisualFlow,
  );
  ...
  const stopVoiceRef = useRef<
    (reason?: "user" | "escape" | "limit" | "device") => Promise<void>
  >(async () => undefined);
  ...
  const activeCapture =
    [
      ...
      "ready_for_review",
      "recoverable_error",
    ].includes(status.stage) && status.platform === "web";
  ...
  const cycleStartedAt =
    status.context?.cycleStartedAt ?? launchResolution?.cycleStartedAt;
  const participantLabel = cycleParticipantLabel(
    cycleParticipant,
    cycleStartedAt,
  );
  ...
  const noteOpen =
    annotationFlow.phase === "choosing" || annotationFlow.phase === "composing";
  ...
  const dockDetail =
    ...
    agentName &&
    harnessDeliveryState === "acknowledged"
      ? `${agentName} recebeu o contexto citado e retomou o trabalho.`
      : status.stage === "ready_for_review" &&
          agentName &&
          harnessDeliveryState === "failed"
  ...
  const acceptLaunch = useCallback(
    async (launch: DesktopCaptureLaunch) => {
      ...
      if (acceptingLaunch.current) {
        setFeedback({
          ...
          message:
            "Aguarde a abertura atual terminar antes de abrir outro convite.",
        });
        return;
      }
      ...
      if (captureOwnsLocalState && current.context) {
        const sameCycle =
          current.context.scenarioId === launch.loopId &&
        ...
        setFeedback(
          sameCycle
            ? {
                ...
                message:
                  "Continue a captura atual; o tempo e suas evidências foram preservados.",
              }
            : {
                tone: "warning",
                title: "Conclua o teste atual primeiro",
                message:
                  "Finalize a captura em andamento antes de abrir outro teste.",
              },
        );
        return;
      }
    ...
    (link: DesktopWorkspaceLink) => {
      ...
      if (
        ...
        setFeedback({
          ...
          message:
            "A captura atual continua protegida. Depois de finalizá-la, troque o cliente novamente na Web.",
        });
        return;
      }
      try {
  ...
  useEffect(() => {
    ...
    void window.voidrCapture.capture
      .status()
  ...
  useEffect(
    () => () => {
      if (annotationNoticeTimer.current)
        window.clearTimeout(annotationNoticeTimer.current);
    },
    [],
  );
  ...
  useEffect(
    () =>
      window.voidrCapture.capture.onAnnotationSync((event) => {
        ...
        if (event.state === "synced") {
          ...
          annotationNoticeTimer.current = window.setTimeout(
        ...
        if (event.state === "pending") {
          setAnnotationNotice({
            ...
            message:
              "A sincronização será retomada automaticamente quando a conexão responder.",
          });
          annotationNoticeTimer.current = window.setTimeout(
            () => setAnnotationNotice(undefined),
            8_000,
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.voidrCapture.capture.onLaunch(
      (launch) => void acceptLaunch(launch),
    );
  ...
  useEffect(() => {
    localStorage.setItem(
      "voidr.capture.runtime",
      serializeWorkspaceBinding(runtime),
    );
    setWorkspaceIdentity(undefined);
    setWorkspaceAuthError("");
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () =>
        setStatus((current) => ({
          ...current,
          elapsedMs: current.elapsedMs + 500,
        })),
      500,
    );
  ...
  useEffect(() => {
    void window.voidrCapture.capture
      .setControlPanel(controlPanelMode)
      .catch(() => undefined);
  }, [controlPanelMode]);
  ...
  useEffect(() => {
    ...
    const update = () => {
      setFinalizationElapsedMs(
        Date.now() - (finalizationStartedAt.current ?? Date.now()),
      );
    };
    update();
  ...
  const startWorkspaceLoop = async (loopId: string) => {
    ...
    setFeedback(undefined);
    try {
      const launch = await window.voidrCapture.workspace.startCycle(
        runtime,
        loopId,
      );
  ...
  const prepareAndStart = () =>
    run(async () => {
      ...
      await window.voidrCapture.capture.prepareWeb({
  ...
  const beginAnnotation = async (kind: AnnotationKind) => {
    if (
      busy ||
      ["selecting", "saving"].includes(annotationFlowRef.current.phase)
    )
      return;
    if (annotationNoticeTimer.current)
      window.clearTimeout(annotationNoticeTimer.current);
    ...
    setAnnotationNotice({
      ...
      active: true,
      title:
        kind === "element" ? "Selecione um elemento" : "Selecione uma região",
      message:
        kind === "element"
  ...
  const cancelAnnotation = useCallback(
    async (destination: "closed" | "choosing" = "closed") => {
      ...
      transitionAnnotation({
        type: destination === "choosing" ? "BACK" : "CLOSE",
      });
      setAnnotationNotice(undefined);
  ...
  useEffect(
    () =>
      window.voidrCapture.capture.onTargetPointerDown(() => {
        if (
  ...
  useEffect(
    () =>
      window.voidrCapture.capture.onSelectionInvalidated((selection) => {
        if (selection.owner === "voice") {
          ...
          if (
            voice.phase === "sending" ||
            (voice.phase === "error" && voice.sendAttempted)
          ) {
            return;
          }
          if (voice.phase !== "reviewing" && voice.phase !== "error") return;
          const message =
            "A página mudou. Selecione a área novamente. Sua gravação foi preservada.";
          voiceVisualAttempt.current += 1;
          transitionVoiceVisual({
        ...
        setAnnotationNotice({
          ...
          message:
            "Escolha o elemento ou a região novamente. Seu texto foi preservado.",
        });
  ...
  useEffect(
    () =>
      window.voidrCapture.capture.onSelectionCancelled((selection) => {
        if (selection.owner === "voice") {
  ...
  const restoreVoiceRetakeFallback = useCallback(
  ...
  const cancelVoiceRequest = useCallback(() => {
    ...
    voiceAttempt.current += 1;
    const restored = restoreVoiceRetakeFallback(
      "A gravação anterior foi preservada.",
    );
    if (!restored) {
      transitionVoice({ type: "REQUEST_CANCELLED", attempt: flow.attempt });
    }
    if (!restored && voiceVisualFlowRef.current.phase !== "none")
      void clearVoiceRegion();
  ...
  const stopVoice = useCallback(
    async (_reason: "user" | "escape" | "limit" | "device" = "user") => {
      const flow = voiceFlowRef.current;
      const capture = voiceCaptureRef.current;
      if (flow.phase !== "recording" || !capture || voiceStopInFlight.current)
        return;
      ...
      try {
        ...
        transitionVoice({
          ...
          ...(!canSend
            ? {
                warning:
                  "A gravação ficou curta demais. Ouça se quiser e grave novamente.",
              }
            : {}),
        });
      } catch (error) {
  ...
  const retakeVoice = useCallback(async () => {
    ...
    if (!draft || (flow.phase !== "reviewing" && flow.phase !== "error"))
      return;
    voiceDraftRef.current = undefined;
  ...
  const sendVoice = useCallback(async () => {
    ...
    const visualSelectionId = voiceVisualSelectionId(
      voiceVisualFlowRef.current,
    );
    ...
    try {
      ...
      if (voiceSendInFlight.current === segmentId) {
        ...
        const visualUnavailable = rawMessage.includes(
          "[VOICE_VISUAL_UNAVAILABLE]",
        );
        const definitiveFailure = rawMessage.includes(
          "[VOICE_SEND_DEFINITIVE]",
        );
  ...
  useEffect(() => {
    ...
    const update = () => {
      const value = Math.min(
        MAX_VOICE_DURATION_MS,
        Date.now() - voiceStartedAtWall.current,
      );
      setVoiceElapsedMs(value);
      if (value >= MAX_VOICE_DURATION_MS - 750)
        void stopVoiceRef.current("limit");
    };
    update();
  ...
  useEffect(() => {
    const automationCapture = window.voidrCapture
      .capture as typeof window.voidrCapture.capture & {
      onVoiceDraftForTest?: (
        callback: (input: { pcmBase64: string }) => void,
      ) => () => void;
    };
    ...
    return automationCapture.onVoiceDraftForTest(({ pcmBase64 }) => {
      if (
        ...
        voiceHasPendingWork(voiceFlowRef.current)
      )
        return;
      disposeVoiceDraft();
  ...
  const refreshDevices = () =>
    run(async () => {
      ...
      if (!selectedDevice && result.devices[0]?.serial)
        setSelectedDevice(result.devices[0].serial);
      setFeedback({
        tone: result.devices.length ? "success" : "warning",
        title: result.devices.length
          ? "Device conectado"
  ...
  const discoverSessions = () =>
    run(async () => {
      ...
      const found =
        await window.voidrCapture.mobile.discoverSessions(selectedDevice);
      setSessionIds(found);
      ...
      setFeedback(
        found.length
          ? {
              tone: "success",
              title: "Captura mobile encontrada",
              message: `${found.length} captura(s) pronta(s) para envio.`,
            }
          : {
              tone: "warning",
              title: "Ainda não há captura finalizada",
              message: "Conclua o fluxo no app e tente novamente.",
            },
      );
    });
  ...
  const loadMobileLoops = () =>
    run(async () => {
      const values = (await window.voidrCapture.mobile.listVerifications(
        runtime,
      )) as MobileVerification[];
      const mobile = values.filter(
        (item) =>
          String(item.applicationType ?? item.platform ?? "").toUpperCase() ===
          "MOBILE",
      );
      ...
      setFeedback({
        ...
        message: `${count} ${count === 1 ? "teste disponível" : "testes disponíveis"}.`,
      });
    });
  ...
  const attachMobile = () =>
    run(async () => {
      if (!verificationId || !sessionId)
        throw new Error("Selecione o teste e a captura do app.");
      const current = (await window.voidrCapture.mobile.verificationStatus(
        runtime,
        verificationId,
      ...
      if (!Number.isInteger(lifecycleVersion) || lifecycleVersion < 0)
        throw new Error(
          "Não foi possível preparar este teste. Atualize e tente novamente.",
        );
      await window.voidrCapture.mobile.attachSession({
        verificationId,
        sessionId,
        lifecycleVersion,
        runtime,
      ...
      setFeedback({
        ...
        message: "A Voidr está organizando o que foi capturado.",
      });
    });
  ...
  const protectVoicePending = (destination: string): boolean => {
    ...
    const recordingMessage = ["requesting", "recording", "stopping"].includes(
      flow.phase,
    );
      ...
      if (
        ...
        try {
          ...
          setFeedback({
            ...
            message:
              "As evidências foram salvas, mas a plataforma não abriu automaticamente. Use “Consolidar e resolver” para continuar.",
          });
        }
      }
    });
        ...
        <div className="capture-topbar-context">
          {status.context ? (
            <>
              ...
              <code>
                {launchResolution?.roundId ? "Rodada de validação · " : ""}
                {status.context.cycleNumber
                  ? `Teste #${status.context.cycleNumber}`
                  : status.context.cycleId.slice(0, 8)}
              </code>
            </>
            ...
            <button
              ...
              onClick={() =>
                void window.voidrCapture.workspace.openPlatform(runtime)
              }
            >
            ...
            <div className="capture-sidebar-footer">
              {workspaceIdentity && (
                <div className="capture-account-card">
                  ...
                  <strong title={workspaceIdentity.user.email}>
                    {workspaceIdentity.user.name}
                  </strong>
                  <small title={workspaceIdentity.user.email}>
              ...
              <div className="capture-first-steps">
                ...
                <p>
                  Escolha um teste para ver feedback, replay e sinais técnicos.
                </p>
              </div>
              {captureChannel !== "production" && (
                <button
            ...
            <main className="capture-home">
              <section className="capture-intro">
                <div>
                  ...
                  <p>
                    Abra o ambiente, registre o que encontrar e envie tudo para
                    o Loop.
                  </p>
                </div>
              ...
              {mode === "web" ? (
                <Panel
                  ...
                  title="Capture uma aplicação Web"
                  subtitle="Use um link emitido pela Voidr ou pelo agente conectado ao seu fluxo."
                >
                  <div className="capture-form">
                    ...
                    <textarea
                      id="recording-url"
                      value={recordingUrl}
                      onChange={(event) => setRecordingUrl(inputValue(event))}
                      placeholder="Cole o link seguro aqui…"
                      rows={2}
                      spellCheck={false}
                    />
                    <div className="capture-trust-row">
                      <span>
                        <Check size={12} /> Credenciais removidas antes de abrir
                      </span>
                      <span>
                        <Check size={12} /> Replay salvo na Voidr
                      </span>
                    </div>
                    <Button
                      variant="primary"
                ...
                <Panel
                  className="capture-primary-panel mobile-flow"
                  title="Capture um app Android"
                  subtitle="Conecte o device, execute a jornada e envie a captura para a verificação."
                >
                  <div
                    className={`mobile-step${selectedDevice ? " complete" : " active"}`}
                  >
                    <StepNumber number={1} complete={Boolean(selectedDevice)} />
                    <div className="mobile-step-content">
                      <div className="mobile-step-heading">
                        <div>
                          <strong>Conecte um device</strong>
                          <span>
                            Use um device físico ou emulador disponível neste
                            computador.
                          </span>
                        </div>
                        <Button
                      ...
                      <select
                        aria-label="Device Android"
                        value={selectedDevice}
                        onChange={(event) => {
                          setSelectedDevice(event.currentTarget.value);
                          setMobileAppOpened(false);
                        }}
                      >
                  ...
                  <div
                    className={`mobile-step${mobileAppOpened ? " complete" : selectedDevice ? " active" : ""}`}
                  >
                    <StepNumber number={2} complete={mobileAppOpened} />
                    <div className="mobile-step-content">
                      <div className="mobile-step-heading">
                        <div>
                          <strong>Abra o app</strong>
                          <span>
                            Execute a jornada normalmente e finalize a captura
                            dentro do app.
                          </span>
                        </div>
                      </div>
                      <div className="mobile-inline-action">
                        <label>
                          <span>Identificador do app</span>
                          <input
                            value={packageName}
                            onChange={(event) =>
                              setPackageName(inputValue(event))
                            }
                        ...
                        <Button
                          icon={<Play size={13} />}
                          disabled={
                            !selectedDevice || !packageName.trim() || busy
                          }
                          onClick={() =>
                            run(async () => {
                              await window.voidrCapture.mobile.launch({
                                serial: selectedDevice,
                                packageName,
                              });
                              setMobileAppOpened(true);
                              setFeedback({
                                tone: "info",
                                title: "App aberto",
                                message:
                                  "Execute a jornada e finalize a captura dentro do app.",
                              });
                            })
                          }
                        >
                  ...
                  <div
                    className={`mobile-step${sessionId && verificationId ? " complete" : mobileAppOpened ? " active" : ""}`}
                  >
                    <StepNumber
                      number={3}
                      complete={Boolean(sessionId && verificationId)}
                    />
                    <div className="mobile-step-content">
                      <div className="mobile-step-heading">
                        <div>
                          <strong>Envie as evidências</strong>
                          <span>
                            Selecione a captura finalizada e a verificação que
                            receberá o contexto.
                          </span>
                        </div>
                      ...
                      <div className="mobile-select-grid">
                        <label>
                          ...
                          >
                            <option value="">Selecione a captura</option>
                            {sessionIds.map((id) => (
                        ...
                        <label>
                          <span>Teste do Loop</span>
                          <select
                            value={verificationId}
                            onChange={(event) =>
                              setVerificationId(event.currentTarget.value)
                            }
                          >
                            <option value="">Selecione o teste</option>
                            {mobileLoops.map((item) => {
                              const id = String(
                                item.verificationId ?? item.id ?? "",
                              );
                              return (
                                <option key={id} value={id}>
                                  {String(
                                    item.mission ?? item.name ?? "Teste mobile",
                                  )}{" "}
                                  · {id.slice(0, 8)}
                                </option>
                              );
                      ...
                      <div className="mobile-step-actions">
                        <Button
                          ...
                          disabled={busy || !verificationId || !sessionId}
                          onClick={attachMobile}
                        >
                          Enviar para a Voidr
                        </Button>
                        {mobileContext && (
                ...
                >
                  <div className="capture-form">
                    ...
                    <input
                      value={
                        launchResolution?.surface === "api"
                          ? launchResolution.targetUrl
                          : ""
                      }
                      readOnly
                    ...
                    <div className="capture-trust-row">
                      <span>
                        <Check size={12} /> Headers sensíveis redigidos
                      </span>
                      <span>
                        <Check size={12} /> Bodies persistidos por referência
                      </span>
                    </div>
                    <Button
                      variant="primary"
                    ...
                    <p className="capture-inline-note">
                      A captura de API ainda não está disponível nesta versão do
                      app.
                    </p>
                  </div>
                </Panel>
              ...
              <section className="capture-utility-row">
                <button
                  ...
                  disabled={busy}
                >
                  <Wrench size={13} /> Verificar ambiente
                </button>
      ...
      {activeCapture && (
        <footer className="capture-dock">
          <div className="dock-state">
            {finalizing ? (
              <VoidrMark size={30} active />
            ) : (
              <StatusDot live={recording} />
            )}
            {recording && annotationNotice ? (
              <div
                className={`annotation-notice tone-${annotationNotice.tone}`}
                aria-live="polite"
              >
                <strong>
                  {annotationNotice.active && (
                    <Loader2 className="spin" size={12} />
                  )}
                  {!annotationNotice.active &&
                    annotationNotice.tone === "success" && <Check size={12} />}
                  {annotationNotice.title}
                </strong>
                {annotationNotice.message && (
                  <span>{annotationNotice.message}</span>
                )}
              </div>
          ...
          {recording && pendingAnnotations > 0 && (
            <div
              className="annotation-sync-state"
              role="status"
              aria-live="polite"
            >
              <Loader2 className="spin" size={12} />
              <span>
                {pendingAnnotations === 1
                  ? "1 nota protegida · sincronizando"
                  : `${pendingAnnotations} notas protegidas · sincronizando`}
              </span>
            </div>
          )}
          <div className="dock-actions">
            ...
            {recording && (
              <Button
                ...
                size="sm"
                variant={
                  annotationFlow.phase === "selecting"
                    ? "secondary"
                    : annotationActive
                      ? "primary"
                      : "secondary"
                ...
                disabled={busy || annotationSaving}
                onClick={() => void toggleAnnotationPanel()}
              >
                {annotationFlow.phase === "selecting"
                  ? "Cancelar seleção"
                  : "Nota"}
            ...
            {recording && (
              <Button
                size="sm"
                variant={
                  voiceFlow.phase === "recording" ? "danger" : "secondary"
                }
                icon={
                  voiceFlow.phase === "recording" ? (
                    <Square size={12} />
                  ) : (
                    <Mic size={13} />
                  )
                ...
                disabled={
                  busy ||
                  annotationSaving ||
                  ["requesting", "stopping", "sending"].includes(
                    voiceFlow.phase,
                  )
                }
                onClick={() => void toggleVoiceWithAnnotationCleanup()}
              >
                {voiceFlow.phase === "recording" ? "Parar" : "Voz"}
              </Button>
            )}
            {recording && (
              <Button
            ...
            {status.stage === "recoverable_error" && (
              <Button
                size="sm"
                variant="primary"
                icon={<RefreshCw size={13} />}
            ...
            {["processing", "ready_for_review"].includes(status.stage) &&
              status.context && (
                <Button
                  size="sm"
                  variant="primary"
                  ...
                  onClick={() =>
                    void window.voidrCapture.openCycle({
                      platformUrl: runtime.platformUrl,
                      loopId: status.context!.scenarioId,
                      cycleId: status.context!.cycleId,
                      destination: "consolidated",
                      agent: "codex",
                    })
                  }
                >
                  Consolidar e resolver
                </Button>
              )}
            {status.stage === "ready_for_review" && (
              <Button
                ...
                onClick={() =>
                  run(async () => {
                    await window.voidrCapture.capture.reset();
                  })
                }
              >
                Nova captura
          ...
          {noteOpen && recording && (
            <section
              className={`dock-note${annotationFlow.phase === "composing" ? " dock-note-composer" : ""}`}
              role="dialog"
              aria-modal="false"
                ...
                <>
                  <header>
                    ...
                    <div className="annotation-header-actions">
                      <button
                        type="button"
                        aria-label="Voltar para os tipos de nota"
                        title="Voltar"
                        onClick={() => void cancelAnnotation("choosing")}
                      >
                        <ArrowLeft size={13} />
                      </button>
                      <button
                        type="button"
                        aria-label="Descartar anotação"
                        title="Descartar"
                        onClick={() => void cancelAnnotation("closed")}
                      >
                        <X size={13} />
                      </button>
                    </div>
                  ...
                  <div className="annotation-capture-row">
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>Selecione algo na tela</small>
                      </span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>Arraste sobre uma área</small>
                      </span>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>
                        <strong>Tela</strong>
                        <small>Capture o viewport atual</small>
                      </span>
                    </button>
                  </div>
                </>
          ...
          {evidenceOpen && recording && (
            <EvidenceInspector
              ...
              label={
                evidenceItems.find((item) => item.key === evidenceOpen)
                  ?.label ?? "Evidências"
              }
              signals={(status.recentSignals ?? []).filter(
                (signal) => signal.category === evidenceOpen,
              )}
              total={status.evidence[evidenceOpen]}
              onClose={() => {
                setEvidenceOpen(undefined);
                void window.voidrCapture.capture.setControlPanel("default");
              }}
            />
          )}
          {finalizing && (
            <FinalizationProgress
              stage={status.stage}
              agentName={agentName}
              elapsedMs={finalizationElapsedMs}
            />
          )}
        </footer>
      )}

      {feedback && !activeCapture && (
        <Toast {...feedback} onClose={() => setFeedback(undefined)} />
      )}
    </div>
  );
}
...
function VoicePanel({
  ...
  const visualLocked =
    flow.phase === "sending" || (flow.phase === "error" && flow.sendAttempted);
  return (
    <section
      className={`dock-voice phase-${flow.phase}`}
      role="dialog"
      aria-modal="false"
    ...
    >
      {flow.phase === "requesting" && (
        <>
          <div className="voice-state-icon">
      ...
      {flow.phase === "recording" && (
        <>
          ...
          <div className="voice-copy">
            ...
            <small>
              Explique o que aconteceu. Depois, você poderá selecionar uma área.
            </small>
          </div>
        ...
        <>
          ...
          {previewUrl && (
            <audio
              ...
              preload="metadata"
              src={previewUrl}
              aria-label="Revisar nota de voz"
            />
          )}
          <div
            className={`voice-visual-context${hasVisual ? " selected" : ""}`}
          >
            ...
            <div>
              <strong>
                {hasVisual ? "Área selecionada" : "Área da tela (opcional)"}
              </strong>
              <small>
                {hasVisual
                  ? visualLocked
          ...
          {notice && flow.phase !== "error" && (
            <p className="voice-guard" role="status">
              {notice}
            </p>
          )}
      ...
      {flow.phase === "unavailable" && (
        <>
          <div className="voice-state-icon tone-error">
            <Mic size={16} />
          </div>
          <div className="voice-copy">
            <span>Nota de voz</span>
          ...
          <Button size="sm" variant="primary" onClick={onRetake}>
            Tentar novamente
          </Button>
        </>
      )}
      {flow.phase === "success" && (
        <>
          <div className="voice-state-icon tone-success">
          ...
          <div className="voice-copy voice-success-copy">
            ...
            <small>
              A transcrição já faz parte das evidências deste teste.
            </small>
          </div>
          <Button size="sm" variant="ghost" onClick={onClose}>
            Fechar
        ...
        {signals.length ? (
          ...
          <div className="dock-evidence-empty">
            ...
            <div>
              ...
              <span>
                {category === "requests"
                  ? "As próximas requisições aparecerão aqui com método, status e duração."
                  : "Continue a jornada; a Voidr adicionará os eventos automaticamente."}
              </span>
            </div>
          </div>
        )}
...
function FinalizationProgress({
  ...
  return (
    <section
      className="finalization-progress"
      aria-label="Progresso da finalização"
      aria-live="polite"
    >
      {finalizationStages.map((item, index) => {
        const current = index === active;
        const title =
          index === 3 && agentName ? `Entregando ao ${agentName}` : item.title;
        const detail =
          current && delayed
            ? stage === "processing"
        ...
        return (
          <div
            key={item.id}
            className={`finalization-step${index < active ? " done" : ""}${current ? " active" : ""}`}
          >
...
function StepNumber({
  ...
  return (
    <span className={`mobile-step-number${complete ? " complete" : ""}`}>
    ...
    >
      <div className="doctor-list">
        ...
        {result.services.map(
          (check: {
            ...
            <div key={check.service}>
              ...
              <span>{check.ok ? `${check.latencyMs} ms` : check.detail}</span>
              <Badge tone={check.ok ? "success" : "error"}>
                {check.ok ? "Pronto" : "Indisponível"}
              </Badge>
            </div>
          ),
        )}
        <div>
          ...
          <span>{result.android.message}</span>
          <Badge tone={result.android.available ? "success" : "warning"}>
            {result.android.available
              ? `${result.android.devices.length} device(s)`
              : "Ação necessária"}
          </Badge>
        </div>
...
function RuntimeSettings({
  ...
  const field = (key: keyof LocalRuntimeConfig, label: string) => (
    <label>
      {label}
      <input
        type={key === "localDevKey" ? "password" : "text"}
        value={String(runtime[key])}
        onChange={(event) =>
          onChange({ ...runtime, [key]: event.currentTarget.value })
        }
      />
    </label>
  );
  return (
    <Panel
      ...
      subtitle="Essas credenciais ficam isoladas da aplicação capturada."
    >
      <div className="runtime-grid">
        {field("serviceUrl", "Service")}
        {field("collectorUrl", "Collector")}
        {field("collectorScriptUrl", "Collector script")}
