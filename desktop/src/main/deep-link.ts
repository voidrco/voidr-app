...
export function parseDesktopCaptureLaunch(input: string): DesktopCaptureLaunch {
  ...
  if (
    ...
    (url.searchParams.has("access") &&
      url.searchParams.get("access") !== "participant") ||
    ...
    throw new Error(
      "O link do Voidr Capture está incompleto ou não é suportado.",
    );
  }
...
function fromBase64Url(value: string): string {
  if (!/^[A-Za-z0-9_-]+$/.test(value))
    throw new Error("Loop bootstrap is malformed");
...
export function parseLoopLaunch(input: string): SecretLoopLaunch {
  if (!isTrustedWebUrl(input)) {
    throw new Error(
      "A captura exige HTTPS ou HTTP em loopback, sem credenciais na URL.",
    );
  }
  const url = new URL(input);
  if (
  ...
  if (
    url.searchParams.get("voidr_bootstrap") === "v2" &&
    fragment.startsWith(V2)
  ) {
    ...
    url.searchParams.get("voidr_bootstrap") === "v1" &&
    fragment.startsWith(V1)
  ) {
    const envelope = JSON.parse(
      fromBase64Url(normalizeTransport(fragment.slice(V1.length))),
    ) as {
      token?: unknown;
    ...
    if (!cycleId && typeof envelope.cycleId === "string")
      cycleId = envelope.cycleId;
    originalHash =
      typeof envelope.originalHash === "string" ? envelope.originalHash : "";
    transportVersion = "v1";
  ...
  if (safeUrl.includes(token))
    throw new Error("A autorização não pôde ser removida da URL.");
